...
// ─── Platform Process Flow (inside blue strip) ─────────────────────────────
    ...
    <div ref={ref} className="flex items-center gap-0">
      {PROCESS_STEPS.map((step, i) => (
        <div key={step.label} className="flex items-center">
          {/* Step: icon + label as a single unit */}
          <motion.div
            className="flex items-center gap-[7px] relative z-10"
            initial={{ opacity: 0 }}
            animate={isInView ? { opacity: 1 } : {}}
            transition={{ duration: 0.6, delay: i * 0.15 + 0.2, ease: EASE }}
          >
            <HugeiconsIcon icon={step.icon} size={16} color="rgba(255,255,255,0.55)" strokeWidth={1.5} />
            <span className="font-['Google_Sans',sans-serif] text-[12px] text-white/75 font-normal whitespace-nowrap tracking-[0.01em]">
              {step.label}
            </span>
          </motion.div>

          {/* Connector: line + comet pulse + chevron */}
          {i < PROCESS_STEPS.length - 1 && (
            <div className="relative flex items-center" style={{ width: 64, marginLeft: 16, marginRight: 16 }}>
              {/* Static track */}
              <div className="absolute top-1/2 -translate-y-1/2 w-full h-[1px] rounded-full bg-white/[0.08]" />

              {/* Comet pulse — single bright dot with a fading tail */}
              <motion.div
                className="absolute top-1/2 -translate-y-1/2 pointer-events-none"
                style={{ left: 0, width: '100%', height: 3, overflow: 'hidden' }}
              >
                <motion.div
                  ...
                  style={{
                    ...
                    height: 3,
                    borderRadius: 2,
                    background: 'linear-gradient(90deg, transparent 0%, rgba(8,197,240,0.4) 40%, rgba(255,255,255,0.85) 85%, rgba(255,255,255,0.95) 100%)',
                    boxShadow: '0 0 6px 1px rgba(8,197,240,0.3)',
                  }}
                  animate={isInView ? {
                    left: ['-28px', '64px'],
                  } : {}}
              ...
              >
                <path d="M1 1l2.5 3L1 7" stroke="white" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round" />
              </motion.svg>
            </div>
          )}
        </div>
...
function ProductAccordionCards() {
  ...
  // Platform strip target positions (spread across full width to match cards)
  const platformLeft = 150;
  const platformRight = 850;
  const platformTargets = [150, 383, 617, 850];
  ...
  return (
    <div>
      ...
      {/* Platform strip — full width, logo left + process flow right */}
      <div
        className="hidden lg:flex rounded-[14px] items-center justify-between py-[18px] px-8 relative overflow-hidden"
        style={{
          width: '100%',
          background: 'linear-gradient(135deg, #0A3A8F 0%, #056BF1 50%, #0480E8 100%)',
          boxShadow: '0 8px 32px rgba(5,107,241,0.25), 0 2px 8px rgba(0,0,0,0.1), inset 0 1px 0 rgba(255,255,255,0.1)',
      ...
      >
        ...
        <div
          ...
          style={{
            background: 'radial-gradient(ellipse 60% 100% at 20% 50%, rgba(255,255,255,0.04) 0%, transparent 70%)',
          }}
        />

        {/* Left: CleanStart logo + Platform label */}
        <div className="relative flex items-center gap-3">
          <svg width="130" height="27" viewBox="0 0 595.93 123.68" fill="none">
          ...
          <span
            className="font-['Google_Sans',sans-serif] text-[11px] text-white/50 font-medium tracking-[0.08em] uppercase"
          >
            Platform
          </span>
        </div>

        {/* Right: CleanCompile → Build → Vault process flow */}
    ...
    <section className="bg-white">
      ...
      <div className="bg-[#056BF1] px-4 md:px-8 lg:px-[50px] py-12 md:py-[100px]">
        ...
        <div className="max-w-[1340px] mx-auto flex flex-col gap-12 md:gap-16">
          ...
          {/* Stats — tight masonry grid, white cards, packed together */}
          <div className="rounded-[14px] overflow-hidden bg-white/[0.06] p-[6px]">
            {/* Desktop: 3-col top row + 2-col bottom row */}
            <div className="hidden md:flex flex-col gap-[6px]">
              {/* Top row — 3 cards */}
              <div className="grid grid-cols-3 gap-[6px]">
                {STATS.slice(0, 3).map((stat, i) => (
                  <div key={stat.label} style={{ height: 130 }}>
                    <StatCard stat={stat} delay={i * 0.06} />
                  </div>
                ))}
              </div>
              {/* Bottom row — 2 cards */}
              <div className="grid grid-cols-2 gap-[6px]">
                {STATS.slice(3).map((stat, i) => (
                  <div key={stat.label} style={{ height: 130 }}>
                    <StatCard stat={stat} delay={(i + 3) * 0.06} />
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Mobile: 2-col grid */}
            <div className="grid grid-cols-2 gap-[6px] md:hidden">
              {STATS.map((stat, i) => (
                <div key={stat.label} style={{ height: 120 }}>
                  <StatCard stat={stat} delay={i * 0.06} />
                </div>
              ))}
            </div>
