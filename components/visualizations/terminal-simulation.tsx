...
/* ════════════════════════════════════════════════
   PHASES
   ════════════════════════════════════════════════
   0  idle         → cursor appears, blinks        600ms
   1  typing       → types FROM python:3.12.0     1800ms
   2  codeReveal   → rest of code fades in          800ms
   3  scrollDown   → editor scrolls to bottom      1600ms
   4  highlightFR  → highlight FROM line           1000ms
   5  transition   → crossfade to CVE               400ms
   6  cveTyping    → type trivy command            1200ms
   7  termReveal   → terminal lines appear         1800ms
   8  totalBar     → red bar + table               2400ms
   9  hold         → pause before loop             1500ms
   ════════════════════════════════════════════════ */
...
/* ════════════════════════════════════════════════════
   MAIN COMPONENT
   ════════════════════════════════════════════════════ */
export function TerminalSimulation({ title }: { title?: string }) {
  ...
  useEffect(() => {
    ...
    switch (phase) {
      ...
      /* Phase 1: type FROM python:3.12.0 */
      case 1: {
        let i = 0;
      ...
      /* Phase 4: highlight FROM line */
      case 4: {
        setScrollY(0); // scroll back up
  ...
  return (
    ...
    >
      ...
      {/* Thin cyan accent line below title bar */}
        ...
        <AnimatePresence mode="wait">
          {isEditor ? (
            ...
            >
              <div style={{ transform: `translateY(-${scrollY}px)`, transition: 'none' }}>
                {DOCKERFILE.map((line, i) => {
                  ...
                  // Determine what text to show on FROM line
                  let fromDisplay = '';
                  if (isFromLine) {
                    fromDisplay = FROM_TEXT.slice(0, typedCount);
                  }

                  // Lines after 3 hidden until codeVisible
                      ...
                      <span className="relative z-10 text-[12px] md:text-[13px] pl-1">
                        {isFromLine ? (
                          <>
                            ...
                            {/* Caret on FROM line during typing */}
                            {phase <= 1 && <Caret />}
                          </>
                        ) : line ? (
                          syn.kw ? (
                        ...
                        {/* Caret at end of file after code revealed */}
            ...
            >
              ...
              {/* python:latest label + red total bar */}
              {showTotal && (
                <motion.div
